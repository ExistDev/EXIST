...
public class EntityPlayerSP extends AbstractClientPlayer {
	...
	public void func_175161_p() {
		EventPreMotionUpdates e = new EventPreMotionUpdates(rotationYaw, rotationPitch, posX, posY, posZ,
				mc.thePlayer.onGround, EventType.PRE);
		EventManager.call(e);
		if (e.isCancelled()) {
			return;
		}
		...
		EventPreMotionUpdates pre = new EventPreMotionUpdates(rotationYaw, rotationPitch, posX, posY, posZ,
				mc.thePlayer.onGround, EventType.POST);
	...
	public void sendChatMessage(String p_71165_1_) {
		if (!Exist.commandManager.processCommand(p_71165_1_)) {
	...
	public void onLivingUpdate() {
		// FastLadder
		if (Exist.getModules(FastLadder.class).isToggled()) {
			if (mc.thePlayer.isMoving() && mc.thePlayer.isOnLadder()) {
				mc.thePlayer.motionY += 0.05F;
			}
		}
		// Glide
		if (Exist.getModules(Glide.class).isToggled()) {
		...
		// NoWeb
		if (Exist.getModules(NoWeb.class).isToggled()) {
			if (mc.thePlayer.isInWeb) {
				if (mc.thePlayer.onGround) {
					mc.thePlayer.motionY = (float) Exist.settingManager.getSetting(Exist.getModules(NoWeb.class), "WebUp")
							.getCurrentValue();
				}
				setSpeed(1);
			}
		...
		if (Exist.getModules(WaterSpeed.class).isToggled()) {
		...
		if (Exist.getModules(InvMove.class).isToggled()) {
			...
			if ((mc.currentScreen instanceof GuiContainer)
					|| mc.currentScreen instanceof GuiGameOver && !mc.playerController.isInCreativeMode()) {
		...
		if (this.isUsingItem() && !this.isRiding()) {
			if (!Exist.getModules(NoSlowdown.class).isToggled()) {
